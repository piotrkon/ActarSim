...
void ActarSimGasDetectorConstruction::SetGasMaterial (G4String mat) {
  ...
  if(mat=="isoC4H10")
    ...
    {
      //Ar (default  1.6642*mg/cm3 STP)
      //density	=(1.6642*293.15*kelvin*pressure)/(1.01325*bar*temperature)*mg/cm3;
    ...
    {
      G4double Percentage_He=0.95;
      G4double Percentage_C4H10=0.05;
	...
	new G4Material("isoC4H10", density, ncomponents=2, kStateGas, temperature, pressure) ;
      ...
      density	= ((Percentage_He*4.0026+Percentage_C4H10*(4*12.0107+10*1.00794))/Vm)*mg/cm3;
      G4Material* HeC4H10 =
      	new G4Material("HeC4H10", density, ncomponents=2, kStateGas, temperature, pressure);

      G4double fractionmass_He=4.0026*Percentage_He/(4.0026*Percentage_He+(4*12.0107+10*1.00794)*Percentage_C4H10);
      G4double fractionmass_iso=(4*12.0107+10*1.00794)*Percentage_C4H10/(4.0026*Percentage_He+(4*12.0107+10*1.00794)*Percentage_C4H10);

      //HeC4H10->AddMaterial( Helium,     fractionmass = 0.35937 ) ;
      //HeC4H10->AddMaterial( isobutane,  fractionmass = 0.64063 ) ;
      HeC4H10->AddMaterial( Helium,     fractionmass = fractionmass_He ) ;
      HeC4H10->AddMaterial( isobutane,  fractionmass = fractionmass_iso ) ;
      gasMaterial = HeC4H10;
